...
class TestVal:
    def test_numpy_unpack(self):
        ...
        val1 = Val(value=np.array([[10, 11], [20, 21], [30, 31]]), var=force)
        print(val1.value)
        unpacked_vals = val1.unpack()
        assert len(unpacked_vals) == 3
        assert_array_equal(unpacked_vals[0].value, np.array([10, 11]))
        assert unpacked_vals[0].var.name == "force x"
        assert_array_equal(unpacked_vals[1].value, np.array([20, 21]))
        assert unpacked_vals[1].var.name == "force y"
        assert_array_equal(unpacked_vals[2].value, np.array([30, 31]))
...
class TestValDict:
    def test_numpy_unpack(self):
        ...
        value = np.array([[10, 11], [20, 21], [30, 31]])
        dct = {}
        dct[force] = value
        dct = ValDict(dct)
        print(dct)
